...
fn memset_non_contiguous(
    ...
    for item in dst_chunk_iter {
        ...
        let dst_host_addr = dst_region.vm_to_host(dst_vm_addr, dst_len as u64)
            .ok_or_else(|| Box::new(SyscallError::InvalidPointer) as Error)?;
    ...
    F: FnMut(*const u8, *const u8, usize) -> Result<T, Error>,
{
    ...
    loop {
        ...
        let (src_host_addr, dst_host_addr) = {
            ...
            (
                src_region.vm_to_host(src_addr, chunk_len as u64)
                    .ok_or_else(|| Box::new(SyscallError::InvalidPointer) as Error)?,
                dst_region.vm_to_host(dst_addr, chunk_len as u64)
                    .ok_or_else(|| Box::new(SyscallError::InvalidPointer) as Error)?,
            )
        };
