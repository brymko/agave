#!/usr/bin/env bash
#
# Run a minimal LOCAL Solana cluster (no network interaction). Ctrl-C to exit.
#
...
export RUST_LOG=${RUST_LOG:-solana=info,agave=info,solana_bpf_loader_program=info,solana_runtime::message_processor=debug} # if RUST_LOG is unset, default to info
export RUST_BACKTRACE=1
# Disable JIT to force interpreter usage
export SOLANA_BPF_JIT_DISABLE=1
...
if [[ "$1" == "--reset" ]]; then
  echo "Cleaning up old ledger and state..."
  rm -rf "$ledgerDir"
  rm -rf "$dataDir"
  # Also remove any existing genesis files
  rm -f spl-genesis-args.sh
fi
...
args=(
  ...
  # LOCAL MODE FLAGS - these make it run offline like solana-test-validator
  --no-snapshot-fetch
  --no-genesis-fetch
  --tpu-enable-udp
  --no-poh-speed-test
  ...
  # Allow single node to make progress
  --allow-private-addr
  # Disable gossip/networking with outside world
...
# Set RPC URL to localhost
solana config set --url http://127.0.0.1:8899

echo "Local validator is running at http://127.0.0.1:8899"
echo "Run 'solana logs' to see program logs"
echo "Run 'solana airdrop 2' to get some SOL"
